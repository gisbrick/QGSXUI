...
function ScheduleConfig({ QGISPRJ, saveProperty, unit, config, properties, setProperties, permissions }) {
    ...
    const loadTables = () =>{
        ...
        for(let i in QGISPRJ.layers){
            if(QGISPRJ.layers[i].classType == "QgsVectorLayer" && "URBEGIS_LAYER_SCHEDULE_CONFIG" in QGISPRJ.layers[i].customProperties){
                values.push({
